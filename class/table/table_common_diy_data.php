...
class table_common_diy_data extends class_table
{
	...
	public function delete($targettplname, $tpldirectory = null) {
		foreach($this->fetch_all($targettplname, $tpldirectory) as $value) {
			...
			@unlink(SERVER_ROOT.'./data/diy/'.$file.'.htm');
			@unlink(SERVER_ROOT.'./data/diy/'.$file.'.htm.bak');
			@unlink(SERVER_ROOT.'./data/diy/'.$file.'_diy_preview.htm');
		}
