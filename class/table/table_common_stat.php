...
class table_common_stat extends class_table
{
	...
	public function updatestat($uid, $type, $primary = 0, $num = 1) {
		...
		if($primary) {
			...
			if(S::t('common_statuser')->check_exists($uid, $nowdaytime, $type)) {
				return false;
			} else {
				S::t('common_statuser')->insert($setarr);
			}
		}
		$num = abs(intval($num));
		if(DB::result_first('SELECT COUNT(*) FROM '.DB::table($this->_table)." WHERE `daytime` = '$nowdaytime'")){
			...
			S::t('common_statuser')->clear_by_daytime($nowdaytime);
