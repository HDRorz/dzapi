...
class table_common_member extends class_table_archive
{
	...
	public function fetch_by_username($username, $fetch_archive = 0) {
		...
		if($username) {
			...
			if(isset($this->membersplit) && $fetch_archive && empty($user)) {
				$user = S::t($this->_table.'_archive')->fetch_by_username($username, 0);
			}
		}
		return $user;
	}
	...
	public function fetch_all_by_username($usernames, $fetch_archive = 1) {
		...
		if(!empty($usernames)) {
			...
			if(isset($this->membersplit) && $fetch_archive && count($usernames) !== count($users)) {
				$users += S::t($this->_table.'_archive')->fetch_all_by_username($usernames, 0);
			}
		}
		return $users;
	}
	...
	public function fetch_uid_by_username($username, $fetch_archive = 0) {
		...
		if($username) {
			...
			if(isset($this->membersplit) && $fetch_archive && empty($uid)) {
				$uid = S::t($this->_table.'_archive')->fetch_uid_by_username($username, 0);
			}
		}
		return $uid;
	}
	...
	public function fetch_all_by_adminid($adminids, $fetch_archive = 1) {
		...
		if($adminids) {
			...
			if(isset($this->membersplit) && $fetch_archive) {
				$users += S::t($this->_table.'_archive')->fetch_all_by_adminid($adminids, 0);
			}
		}
		return $users;
	}
	...
	public function count($fetch_archive = 1) {
		...
		if(isset($this->membersplit) && $fetch_archive) {
			$count += S::t($this->_table.'_archive')->count(0);
		}
	...
	public function fetch_by_email($email, $fetch_archive = 0) {
		...
		if($email) {
			...
			if(isset($this->membersplit) && $fetch_archive && empty($user)) {
				$user = S::t($this->_table.'_archive')->fetch_by_email($email, 0);
			}
		}
		return $user;
	}
	...
	public function fetch_all_by_email($emails, $fetch_archive = 1) {
		...
		if(!empty($emails)) {
			...
			if(isset($this->membersplit) && $fetch_archive && count($emails) !== count($users)) {
				$users += S::t($this->_table.'_archive')->fetch_all_by_email($emails, 0);
			}
		}
		return $users;
	}
	...
	public function count_by_email($email, $fetch_archive = 0) {
		...
		if($email) {
			...
			if(isset($this->membersplit) && $fetch_archive) {
				$count += S::t($this->_table.'_archive')->count_by_email($email, 0);
			}
		}
		return $count;
	}
	...
	public function delete_no_validate($uids) {
		if(($uids = dintval($uids, true))) {
			...
			S::t('common_member_field_forum')->delete($uids);
			S::t('common_member_field_home')->delete($uids);
			S::t('common_member_status')->delete($uids);
			S::t('common_member_count')->delete($uids);
			S::t('common_member_profile')->delete($uids);
			S::t('common_member_validate')->delete($uids);
			return $delnum;
		}
		return false;
	}
	...
	public function insert($uid, $username, $password, $email, $ip, $groupid, $extdata, $adminid = 0) {
		if(($uid = dintval($uid))) {
			...
			S::t('common_member_status')->insert($status, false, true);
			S::t('common_member_count')->insert($count, false, true);
			S::t('common_member_profile')->insert($profile, false, true);
			S::t('common_member_field_forum')->insert($ext, false, true);
			S::t('common_member_field_home')->insert($ext, false, true);
	...
	public function split($splitnum, $iscron = false) {
		...
		if(DB::result_first('SELECT COUNT(*) FROM '.$temptablename)) {
			...
			S::t('common_setting')->delete('memberspliting');
			return true;
		} else {
			DB::query('DROP TABLE '.$temptablename);
			if(!$iscron) {
				$this->switch_keys('enable');
				S::t('common_member_profile')->optimize();
				S::t('common_member_field_forum')->optimize();
				S::t('common_member_field_home')->optimize();
			}
			return false;
		}
	}
	...
	public function switch_keys($type) {
		if($type === 'disable') {
			$type = 'DISABLE';
			S::t('common_setting')->update_batch(array('memberspliting'=>1, 'membersplit'=>1));
		} else {
			$type = 'ENABLE';
			S::t('common_setting')->delete('memberspliting');
		}
