...
class table_common_syscache extends class_table
{
	...
	public function fetch_all($cachenames) {
		...
		if($this->_isfilecache) {
			...
			foreach($cachenames as $cachename) {
				if(!@include_once(SERVER_ROOT.'./data/cache/cache_'.$cachename.'.php')) {
		...
		while($syscache = DB::fetch($query)) {
			...
			if($this->_isfilecache) {
				...
				if(($fp = @fopen(SERVER_ROOT.'./data/cache/cache_'.$syscache['cname'].'.php', 'wb'))) {
	...
	public function insert($cachename, $data) {
		...
		$this->_isfilecache && @unlink(SERVER_ROOT.'./data/cache/cache_'.$cachename.'.php');
	}
